...
J = 1/2 * sum(sum( ( (X*Theta' - Y).*R ).^2 )) + ...
    lambda/2*(sum(sum(Theta.^2,2)) + sum(sum(X.^2,2)));
% =============================================================

% % vectorization 1 on the basis of Tipps
% for i = 1: size(X_grad,1)
%     jdx = find(R(i,:)==1);
%     X_grad(i,:) = (X(i,:)*Theta(jdx,:)' - Y(i,jdx))* Theta(jdx,:) + lambda*X(i,:);
% end
% 
...
% vectorization 2 on the basis of Equation
diff = (X * Theta' - Y);
