...
for i = 1:m
    J = J + (- Y(i,:)*log(a_3(i,:)') -(1 - Y(i,:))*log(1-a_3(i,:)'));
end
J = J/m + lambda/(2*m)*(sum(sum(Theta1(:,2:end).^2,2))+sum(sum(Theta2(:,2:end).^2,2)));
...
for i = 1:m
    ...
    delta_2 = delta_2.*sigmoidGradient([a_1(i,:)*Theta1']');
    
    
    Delta_1 = Delta_1 + delta_2 * a_1(i,:);
